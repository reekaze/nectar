...
class SelectLocationController extends GetxController {
  ...
  void onInit() async {
    ...
    Response response;
    var dio = Dio();
    response = await dio.get("https://wft-geo-db.p.rapidapi.com/v1/geo/countries?offset=60",
        options: Options(headers: {
          "X-RapidAPI-Key": "4602013230msh9701492263a7731p154bebjsn8607685bc37e",
  ...
  getCities() async {
    Response response;
    var dio = Dio();
    response = await dio.get("https://wft-geo-db.p.rapidapi.com/v1/geo/cities?offset=100",
        queryParameters: {"countryIds": selectedCountryCode.value},
        options: Options(
          headers: {'X-RapidAPI-Key': '4602013230msh9701492263a7731p154bebjsn8607685bc37e', 'X-RapidAPI-Host': 'wft-geo-db.p.rapidapi.com'},
        ));
    List<dynamic> data = response.data["data"];
    ...
    if (selectedCountryName.value == "") {
      ...
      try {
        await FirebaseAuth.instance.createUserWithEmailAndPassword(
            email: signUpController.emailController.value.text, password: signUpController.passwordController.value.text);
        ...
        if (e.code == 'weak-password') {
          ...
          Get.snackbar("The accoung already exists for that email", "Please change email");
        }
      } catch (e) {
